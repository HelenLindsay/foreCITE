# Buus code for feature rankplots ----

#'@importFrom ggplot2 ggplot aes facet_wrap geom_hex geom_density labs
...
feature_rankplot_hist <- function(data,
    ...
    data.combined.sum.label <- data.combined.sum %>%
        group_by(wrap) %>%
        summarise(label=paste(paste0(group,": ",
                                     sprintf("%05s",as.character(sum))),
                              collapse="\n"))
    ...
    if(draw.histogram == TRUE){
        p.hist <- ggplot(data.combined, aes(x=value)) +
            scale_x_continuous(trans=trans,
                               limits=c(-1,max(data.combined$value)),
                               expand=c(0.01,0.01)) +
            geom_density(aes(y=..density.. ,linetype=group, fill=group),
                         alpha=0.5, bw=0.35) +
            guides(fill=guide_legend(reverse = TRUE),
                   linetype=guide_legend(reverse = TRUE)) +
            scale_y_continuous(expand=c(0,0)) +
            scale_fill_manual(values=histogram.colors) +
        ...
        if(!is.null(wrap)){
            p.hist <- p.hist +
                geom_text(data=data.combined.sum.label,
                          x = Inf, y = Inf, hjust=1, vjust=1.5,
                          aes(label=label), size=1.5)
            p.hist <- p.hist + facet_wrap( ~wrap)
        } else {
            scale_label <- with(data.combined.sum[order(
                factor(data.combined.sum$group,
                       levels=levels(data.combined$group))),],
                paste0(group,": ",sprintf("%05s",as.character(sum))))

            p.hist <- p.hist +
                scale_linetype_discrete(labels=scale_label) +
                scale_fill_manual(values=histogram.colors, labels=scale_label) +
                theme(legend.position=c(1,1.5),legend.justification=c(1,1),
                      legend.text.align=1, plot.margin=unit(c(0.3,0,0,0),"cm"))
        }
    }
    ...
    p.feature_rankplot <- feature_rankplot(data=data.combined$value,
        ...
        theme(strip.text = element_blank(),
              ...
              axis.text.y=element_blank()) +
        guides(linetype=F, col=guide_legend(override.aes = list(shape = 15)),
               group=F) + ylab("UMI count") + xlab("Cell ranking")

    if(legend == FALSE){
        p.feature_rankplot <- p.feature_rankplot +
    ...
    if(!is.null(gates)){
        p.feature_rankplot <- p.feature_rankplot +
            geom_vline(data=gates,aes(xintercept=gate),
                       col="red", alpha=0.5, linetype="dashed")
    }

    p <- plot_grid(p.hist, p.feature_rankplot, ncol=1,
                   align="v", axis="lr", label_size=7, labels=c(title,""),
                   hjust = 0, vjust=1.1, rel_heights=c(5,15,2))
